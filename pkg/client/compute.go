...
// GetAvailabilityZone returns the virtual machine client to communicate with the wssd agent
func GetAvailabilityZoneClient(serverAddress *string, authorizer auth.Authorizer) (compute_pb.AvailabilityZoneAgentClient, error) {
	conn, err := getClientConnection(serverAddress, authorizer)
	if err != nil {
		log.Fatalf("Unable to get AvailabilityZoneClient. Failed to dial: %v", err)
	}

	return compute_pb.NewAvailabilityZoneAgentClient(conn), nil
}

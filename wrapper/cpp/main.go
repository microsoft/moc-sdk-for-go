...
func KeyvaultKeySignData(serverName *C.char, groupName *C.char, keyvaultName *C.char, keyName *C.char, input *C.char, timeoutInSeconds C.int) *C.char {
	keyClient, err := getKeyvaultKeyClient(C.GoString(serverName))
	// if errror occurs, return an empty string so that caller can tell between error and decrypted blob
	if err != nil {
		return C.CString("")
	...
	var value string
	value = C.GoString(input)
	signingInfo := keyvault.SigningInfo{SignaturePaddingAlgorithm: keyvault.PKCSv1,
		CryptoSignatureAlgorithm: keyvault.SHA256}

	keySigningOperationValue := keyvault.KeySigningOperationValue{Value: &value,
		SigningInfo: signingInfo,
	}
	ksobytes, err := json.Marshal(keySigningOperationValue)
	if err != nil {
		return C.CString("")
	}
	ksostring := string(ksobytes)
	parameters := &keyvault.KeyOperationsParameters{
		Value: &ksostring,
	}

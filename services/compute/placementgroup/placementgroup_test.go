...
func Test_getRpcPlacementGroup(t *testing.T) {
	result, err := getRpcPlacementGroup(nil, "testGroup")
	assert.Error(t, err)
	assert.Nil(t, result)

	pgroup := compute.PlacementGroup{
		Name:                     &name,
	...
	result, err = getRpcPlacementGroup(&pgroup, "group1")
	assert.Error(t, err)
...
func Test_getWssdPlacementGroup(t *testing.T) {
	result, err := getWssdPlacementGroup(nil)
	assert.Error(t, err)
	assert.Nil(t, result)

	pgroup := wssdcloudcompute.PlacementGroup{
		Name:                     name,
		PlatformFaultDomainCount: fdcount,
	...
	result, err = getWssdPlacementGroup(&pgroup)
	assert.Nil(t, err)

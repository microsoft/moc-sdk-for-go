...
// semantics).
func (c *client) UpdateRegisteredIPs(ctx context.Context, locationName, name string, subnetRegisteredIPs []SubnetRegisteredIPs) (subnetPersistedIPs []SubnetRegisteredIPs, failures []IPAddressUpdateFailure, err error) {
	if len(locationName) == 0 {
		return nil, nil, errors.Wrapf(errors.InvalidInput, "LocationName is not specified")
	}
	if len(name) == 0 {
		return nil, nil, errors.Wrapf(errors.InvalidInput, "Name is not specified")
	}

	req := &wssdcloudnetwork.LogicalNetworkIPUpdateRequest{
		LocationName: locationName,
		Name:         name,
		IPUpdates:    subnetsToProto(subnetRegisteredIPs),
	}

	resp, err := c.LogicalNetworkAgentClient.UpdateRegisteredIPs(ctx, req)
	if err != nil {
		return nil, nil, err
	}
...
func subnetsToProto(in []SubnetRegisteredIPs) []*wssdcloudnetwork.LogicalSubnetIPUpdate {
	out := make([]*wssdcloudnetwork.LogicalSubnetIPUpdate, 0, len(in))
	for _, s := range in {
		out = append(out, &wssdcloudnetwork.LogicalSubnetIPUpdate{
